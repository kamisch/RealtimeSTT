if __name__ == "__main__":
    ...
    from PyQt5.QtGui import QColor, QFont, QFontMetrics, QMouseEvent, QPainter
    from PyQt5.QtWidgets import QAction, QApplication, QDesktopWidget, QMenu, QWidget
    ...
    system_prompt_message = {"role": "system", "content": prompt}
    ...
    class TransparentWindow(QWidget):
        ...
        def __init__(self):
            ...
            self.elevenlabs_action.triggered.connect(
                lambda: self.select_engine("Elevenlabs")
            )
        ...
        def select_engine(self, engine_name):
            ...
            if engine_name == "Azure":
                ...
                engine = ElevenlabsEngine(
                    os.environ.get("ELEVENLABS_API_KEY"), model=elevenlabs_model
                )
            else:
                engine = SystemEngine(
                    voice=voice_system,
        ...
        def generate_answer(self):
            ...
            assistant_response = generate_response(
                [system_prompt_message] + history[-max_history_messages:]
            )
            self.stream.feed(assistant_response)
            self.stream.play_async(
                minimum_sentence_length=6, buffer_threshold_seconds=2
            )
        ...
        def update_self(self):
            ...
            self.displayed_user_text, self.user_width = (
                self.return_text_adjusted_to_width(
                    self.user_text, self.user_font, MAX_WIDTH_USER
                )
            )
            self.displayed_assistant_text, self.assistant_width = (
                self.return_text_adjusted_to_width(
                    self.assistant_text, self.assistant_font, MAX_WIDTH_ASSISTANT
                )
            )
            ...
            self.total_height = sum(
                [self.symbol_height, self.user_height, self.assistant_height]
            )

            desktop = QDesktopWidget()
            screen_rect = desktop.availableGeometry(desktop.primaryScreen())
            self.setGeometry(
                screen_rect.right() - self.total_width - 50,
                0,
                self.total_width + 50,
        ...
        def paintEvent(self, event):
            ...
            if self.big_symbol_text:
                painter.drawText(
                    self.total_width - self.symbol_width + 5 + offsetX,
                    offsetY,
                    self.symbol_width,
                    self.symbol_height,
                ...
                painter.drawText(
                    self.total_width - self.symbol_width + 17 + offsetX,
                    offsetY + 10,
                    self.symbol_width,
                    self.symbol_height,
            ...
            user_y = offsetY + 15
            user_color_with_opacity = QColor(
                user_color.red(),
                user_color.green(),
                user_color.blue(),
                self.user_text_opacity,
            )
            ...
            self.drawTextWithOutline(
                ...
                Qt.AlignRight | Qt.AlignTop,
                self.displayed_user_text,
                user_color_with_opacity,
                outline_color_with_opacity,
                2,
            )
            ...
            assistant_color_with_opacity = QColor(
                assistant_color.red(),
                assistant_color.green(),
                assistant_color.blue(),
                self.assistant_text_opacity,
            )
            ...
            self.drawTextWithOutline(
                ...
                assistant_x,
                assistant_y,
                self.assistant_width,
                self.assistant_height,
                Qt.AlignRight | Qt.AlignTop,
                self.displayed_assistant_text,
                assistant_color_with_opacity,
                outline_color_with_opacity,
                2,
            )
