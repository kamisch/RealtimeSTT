...
class AudioInput:
    ...
    def get_supported_sample_rates(self, device_index):
        ...
        max_channels = device_info.get(
            "maxInputChannels"
        )  # Changed from maxOutputChannels
    ...
    def _get_best_sample_rate(self, actual_device_index, desired_rate):
        ...
        try:
            device_info = self.audio_interface.get_device_info_by_index(
                actual_device_index
            )
    ...
    def list_devices(self):
        ...
        try:
            ...
            for i in range(device_count):
                ...
                ):  # Only consider devices with input capabilities
                    supported_rates = self.get_supported_sample_rates(i)
                    print(
                        f"{Fore.LIGHTGREEN_EX}Device {Style.RESET_ALL}{i}{Fore.LIGHTGREEN_EX}: {device_name}{Style.RESET_ALL}"
                    )

                    # Format each rate in cyan
                    if supported_rates:
                        rates_formatted = ", ".join(
                            [
                                f"{Fore.CYAN}{rate}{Style.RESET_ALL}"
                                for rate in supported_rates
                            ]
                        )
                        print(
                            f"  {Fore.YELLOW}Supported sample rates: {rates_formatted}{Style.RESET_ALL}"
                        )
                    else:
                        print(
                            f"  {Fore.YELLOW}Supported sample rates: None{Style.RESET_ALL}"
                        )
    ...
    def setup(self):
        ...
        try:
            ...
            self.device_sample_rate = self._get_best_sample_rate(
                actual_device_index, self.target_samplerate
            )

            if self.debug_mode:
                print(
                    f"Setting up audio on device {self.input_device_index} with sample rate {self.device_sample_rate}"
                )
            ...
            try:
                ...
                if self.debug_mode:
                    print(
                        f"Audio recording initialized successfully at {self.device_sample_rate} Hz"
                    )
                return True
            except Exception as e:
                print(
                    f"Failed to initialize audio stream at {self.device_sample_rate} Hz: {e}"
                )
                return False
    ...
    def resample_audio(self, pcm_data, target_sample_rate, original_sample_rate):
        ...
        if target_sample_rate < original_sample_rate:
            ...
            pcm_filtered = self.lowpass_filter(
                pcm_data, target_sample_rate / 2, original_sample_rate
            )
            resampled = resample_poly(
                pcm_filtered, target_sample_rate, original_sample_rate
            )
        else:
            # Upsampling without low-pass filter
            resampled = resample_poly(
                pcm_data, target_sample_rate, original_sample_rate
            )
        return resampled
