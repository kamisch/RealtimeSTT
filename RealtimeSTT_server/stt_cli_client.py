...
from RealtimeSTT import AudioInput, AudioToTextRecorderClient
...
def main():
    ...
    parser.add_argument(
        "-n", "--norealtime", action="store_true", help="Disable real-time output"
    )
    parser.add_argument(
        "-W", "--write", metavar="FILE", help="Save recorded audio to a WAV file"
    )
    parser.add_argument(
        "-s",
        "--set",
    ...
    parser.add_argument(
        "--data", "--data_url", default=DEFAULT_DATA_URL, help="STT Data WebSocket URL"
    )
    parser.add_argument(
        "--post-silence",
        type=float,
    ...
    def on_realtime_transcription_update(text):
        ...
        if not args.norealtime:
            ...
            if args.speech_end_detection:
                if ends_with_ellipsis(text):
                    if not post_speech_silence_duration == mid_sentence_detection_pause:
                        ...
                        if args.debug:
                            print(
                                f"RT: post_speech_silence_duration for {text} (...): {post_speech_silence_duration}"
                            )
                elif (
                    sentence_end(text)
                    ...
                    if (
                        not post_speech_silence_duration
                        == end_of_sentence_detection_pause
                    ):
                        set_post_speech_silence_duration(
                            end_of_sentence_detection_pause
                        )
                        if args.debug:
                            print(
                ...
                if (
                    len(text_time_deque)
                    >= hard_break_even_on_background_noise_min_texts
                ):
                    ...
                    if (
                        ...
                        and len(first_text)
                        > hard_break_even_on_background_noise_min_chars
                    ):
            ...
            colored_text = (
                f"{Fore.YELLOW}{last_chars}{Style.RESET_ALL}{recording_indicator}\b\b"
            )
            write(colored_text)
