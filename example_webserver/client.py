...
async def send_start_recording(websocket):
    command = {"type": "command", "content": "start-recording"}
...
async def test_client():
    while True:
        try:
            async with websockets.connect(URI, ping_interval=None) as websocket:
                    ...
                    async def check_space_keypress():
                        while True:
                            if keyboard.is_pressed("space"):
                                print(
                                    "Space bar pressed. Sending start-recording message to server."
                                )
                ...
                while True:
                    ...
                    if message_obj["type"] == "realtime":
                        ...
                        clear_console()
                        colored_message = (
                            Fore.YELLOW + message_obj["content"] + Style.RESET_ALL
                        )
            ...
            print(
                "Connection with server closed. Reconnecting in",
                RECONNECT_DELAY,
                "seconds...",
            )
            ...
            print(
                f"An error occurred: {e}. Reconnecting in",
                RECONNECT_DELAY,
                "seconds...",
            )
            await asyncio.sleep(RECONNECT_DELAY)
